...
class TeamLeaderApiController extends Controller
{
    ...
    public function storeAssignAgent(Request $request, DeviceHierarchyAssignmentService $hierarchyService)
    {
        ...
        app(\App\Services\NotificationDispatchService::class)->devicesAssigned(
            $agent,
            Auth::user(),
            $count,
        );
    ...
    public function storeReturnDevices(Request $request, \App\Services\TeamLeaderDeviceReturnService $returnService)
    {
        ...
        try {
            $return = $returnService->createByTeamLeader(
                Auth::user(),
                (int) $validated['product_id'],
                $validated['product_list_ids'],
        ...
        return response()->json([
            'message' => $count === 1
                ? 'Return request sent to regional manager. They must accept before devices leave your custody.'
                : "{$count} devices submitted for return. Regional manager must accept the request.",

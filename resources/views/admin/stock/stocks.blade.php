<x-admin-layout>
    ...
    <div class="admin-prod-page">
        <div class="admin-prod-toolbar">
            <div>
                ...
                <p class="admin-prod-subtitle">
                    @if($tab === 'by-model')
                        Stock in hand per device model, broken down by who currently holds it in the distribution chain.
                    @else
                        Stock buckets for the app: quantities, purchases, and status.
                    @endif
                </p>
            </div>
            @if($tab !== 'by-model')
        ...
        <div class="admin-prod-filter-row mt-4" role="tablist" aria-label="Stock views">
            <a href="{{ route('admin.stock.stocks') }}"
                class="admin-prod-filter-tab {{ $tab === 'stocks' ? 'admin-prod-filter-tab--active' : '' }}"
                role="tab"
                @if($tab === 'stocks') aria-current="page" aria-selected="true" @else aria-selected="false" @endif>
                Stocks
            </a>
            <a href="{{ route('admin.stock.stocks', ['tab' => 'by-model']) }}"
                class="admin-prod-filter-tab {{ $tab === 'by-model' ? 'admin-prod-filter-tab--active' : '' }}"
                role="tab"
                @if($tab === 'by-model') aria-current="page" aria-selected="true" @else aria-selected="false" @endif>
                Stock by Model
            </a>
        </div>

        @if(session('success'))
            <div class="admin-prod-alert admin-prod-alert--success mt-4" role="status">{{ session('success') }}</div>
        @endif
        @if(session('info'))
            <div class="admin-prod-alert admin-prod-alert--warning mt-4" role="status">{{ session('info') }}</div>
        @endif

        @if($tab === 'by-model')
            @include('admin.stock.partials.stock-by-model')
        @else
                            ...
                            @forelse($stocks as $stock)
                                <tr>
                                    <td class="font-semibold">
                                        @if($hasPurchases)
                                            <a href="{{ route('admin.stock.purchase.show', array_merge(['id' => $stock->id], $holder !== '' ? ['holder' => $holder] : [])) }}"

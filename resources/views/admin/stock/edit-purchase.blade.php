...
<x-admin-layout>
    ...
    <div class="admin-prod-page admin-prod-form-wide">
        ...
        {{-- Purchase details (non-payment) --}}
        <div class="admin-clay-panel admin-prod-form-shell overflow-hidden mb-6">
            <div class="admin-prod-form-head">
                <h2 class="admin-prod-form-title">Purchase</h2>
            </div>
            <form action="{{ route($updateRoute, $purchase->id) }}" method="POST" class="admin-prod-form-body" id="purchase-details-form">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="date" name="date" id="date" value="{{ old('date', $purchase->date?->format('Y-m-d') ?? $purchase->date) }}" disabled class="admin-prod-input cursor-not-allowed">
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="text" id="total_amount" readonly class="admin-prod-input font-bold cursor-not-allowed" value="{{ number_format($purchaseTotal, 2) }}">
                    </div>
                ...
                <div class="admin-prod-form-footer !mt-6">
                    <a href="{{ route($listRoute) }}" class="admin-prod-btn-ghost">Back to list</a>
                    <button type="submit" class="admin-prod-btn-primary px-8">Update purchase</button>
                </div>
            </form>
        </div>

        {{-- Payment form (independent — has its own submit, stays on this page after save) --}}
        <div class="admin-clay-panel admin-prod-form-shell overflow-hidden mb-6" id="payment-details">
            <div class="admin-prod-form-head">
                <h2 class="admin-prod-form-title">Payment Details</h2>
            </div>
            <form action="{{ route($updateRoute, $purchase->id) }}" method="POST" class="admin-prod-form-body" id="payment-details-form">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="col-span-1">
                        ...
                        <input type="date" name="paid_date" id="paid_date" value="{{ old('paid_date', $purchase->paid_date?->format('Y-m-d') ?? $purchase->paid_date ?? now()->toDateString()) }}" class="admin-prod-input">
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        <input
                            ...
                            @if($pendingNow <= 0) disabled @endif>
                    ...
                    <div class="col-span-1">
                        ...
                        <select name="payment_option_id" id="payment_option_id" class="admin-prod-select" @if($pendingNow <= 0) disabled @endif>
                        ...
                        <p class="text-xs text-slate-500 mt-1"><strong>Required</strong> when “Paid (this time)” is greater than 0 — pick the bank or other channel the money leaves from.</p>
                        <p id="payment_channel_error" class="mt-2 text-sm font-medium text-red-600 hidden" role="alert"></p>
                    </div>
                </div>

                <div class="admin-prod-form-footer !mt-6">
                    <button type="submit" id="edit_purchase_submit" class="admin-prod-btn-primary px-8" @if($pendingNow <= 0) disabled title="This purchase is already fully paid." @endif>
                        {{ $pendingNow > 0 ? 'Save payment' : 'Fully paid' }}
                    </button>
                </div>
            </form>
        </div>

        {{-- Payment history (outside forms) --}}
    ...
    <script>
        (function() {
            ...
            function updatePendingAmount() {
                if (!paidInput || !pendingEl || pendingBase <= 0) return;
                ...
                pendingEl.value = Math.max(0, pendingBase - payNow).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
            }

            if (paidInput) paidInput.addEventListener('input', updatePendingAmount);
            if (optionSelect) optionSelect.addEventListener('change', updatePendingAmount);
            ...
            if (form) {
                form.addEventListener('submit', function (e) {
                    ...
                    if (submitBtn && submitBtn.disabled) {
                        e.preventDefault();
                        window.alert(errEl && errEl.textContent ? errEl.textContent : 'Cannot save: fix payment channel or amount.');
                        return false;
                    }
                    if (paidInput && !paidInput.disabled && !paidInput.checkValidity()) {
                        e.preventDefault();
                        paidInput.reportValidity();
                        return false;
                    }
                    var payNow = parseFloat(paidInput && paidInput.value) || 0;
                    if (payNow <= 0) {
                        e.preventDefault();
                        window.alert('Enter an amount greater than 0 to save a payment.');
                        return false;
                    }
                });
            }

<x-regional-manager-layout title="Regional inventory & IMEIs">
    <div class="admin-prod-page !pt-4 sm:!pt-6">
        <div class="admin-prod-toolbar !mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <p class="admin-prod-eyebrow">Region</p>
                <h1 class="admin-prod-title">Regional inventory &amp; IMEIs</h1>
                <p class="admin-prod-subtitle">Every device assigned to agents under your team leaders: IMEI, product,
                    branch, team leader, and sale status.</p>
            </div>
            <a href="{{ route('regional-manager.dashboard') }}" class="admin-prod-btn-ghost shrink-0">Back to overview</a>
        </div>
        ...
        <div class="mb-6 admin-clay-panel overflow-hidden p-4 sm:p-5">
            <form method="GET" action="{{ route('regional-manager.region-inventory') }}"
                class="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-end">
                <div class="min-w-[140px] flex-1">
                    <label for="filter_tl" class="admin-prod-label">Team leader</label>
                    <select name="team_leader_id" id="filter_tl" class="admin-prod-input w-full">
                        <option value="">All team leaders</option>
                        @foreach ($teamLeaders as $tl)
                            <option value="{{ $tl->id }}" @selected((string) request('team_leader_id') === (string) $tl->id)>
                                {{ $tl->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="min-w-[140px] flex-1">
                    ...
                    <select name="agent_id" id="filter_agent" class="admin-prod-input w-full">
                        ...
                        @foreach ($agents as $a)
                            <option value="{{ $a->id }}" @selected((string) request('agent_id') === (string) $a->id)>
                                {{ $a->name }} · {{ $a->teamLeader?->name ?? '—' }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="min-w-[140px] flex-1">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('regional-manager.region-inventory') }}" class="admin-prod-btn-ghost">Reset</a>
                </div>
            </form>
        </div>
                ...
                <table class="min-w-[1180px]">
                    <thead>
                        <tr>
                            ...
                            <th class="admin-prod-th">Team leader</th>
                    ...
                    <tbody>
                        @forelse ($rows as $assignment)
                            @php $pl = $assignment->productListItem; @endphp
                            <tr>
                                ...
                                <td class="text-slate-700">{{ $assignment->agent?->teamLeader?->name ?? '—' }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="py-12 text-center text-slate-500">No IMEI assignments for your
                                    hierarchy yet.</td>
                            </tr>
                        @endforelse
                    </tbody>

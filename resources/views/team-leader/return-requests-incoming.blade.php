<x-team-leader-layout title="Return requests from agents">
    <div class="admin-prod-page !pt-4 sm:!pt-6">
        <div class="admin-prod-toolbar !mb-6">
            <div>
                <p class="admin-prod-eyebrow">Team leader</p>
                <h1 class="admin-prod-title">Return requests from agents</h1>
                <p class="mt-1 text-slate-600">Agents send devices back to you here. Accept to receive them into your inventory, or decline if something is wrong.</p>
            </div>
        </div>
            ...
            <table class="js-datatable min-w-full divide-y divide-slate-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-slate-900">From agent</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($returns as $r)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-slate-900">{{ $r->fromAgent->name ?? 'Agent' }}</td>
                            ...
                            <td class="px-4 py-3">
                                @if($r->status === 'pending')
                                    <div class="flex flex-wrap items-center gap-2">
                                        <form method="POST" action="{{ route('team-leader.return-requests.incoming.accept', $r->id) }}" class="inline"
                                        ...
                                        <form method="POST" action="{{ route('team-leader.return-requests.incoming.decline', $r->id) }}" class="inline"
                        ...
                        @if($r->message || $r->recipient_note)
                            <tr class="bg-slate-50/80">
                                <td colspan="5" class="px-4 py-2 text-xs text-slate-600">
                                    @if($r->message)<span class="font-medium text-slate-700">Agent note:</span> {{ $r->message }}@endif
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center text-slate-500">No return requests from agents yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

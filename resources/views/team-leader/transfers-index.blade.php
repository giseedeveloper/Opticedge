<x-team-leader-layout title="Transfer requests">
    <div class="admin-prod-page !pt-4 sm:!pt-6">
        <div class="admin-prod-toolbar !mb-6">
            <div>
                ...
                <p class="mt-1 text-slate-600">Incoming product requests from your regional manager. Accept or decline each request to receive the devices into your inventory.</p>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($transfers as $t)
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <span class="text-slate-900">{{ $t->fromRegionalManager->name ?? 'Regional manager' }}</span>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap items-center gap-2">
                                    @if($t->status === 'pending')
                                        <form method="POST" action="{{ route('team-leader.transfers.accept', $t->id) }}" class="inline"
                                        ...
                                        <form method="POST" action="{{ route('team-leader.transfers.decline', $t->id) }}" class="inline"
                        ...
                        @if($t->message || $t->admin_note)
                            <tr class="bg-slate-50/80">
                                <td colspan="5" class="px-4 py-2 text-xs text-slate-600">
                                    @if($t->message)<span class="font-medium text-slate-700">Regional manager note:</span> {{ $t->message }}@endif
